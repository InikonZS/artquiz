...
import {MapObject, AbstractUnit, InteractiveObject} from "./interactives";
...
export function findClosestBuild(playerPosition:Vector, builds:Array<MapObject>){
  ...
  builds.forEach((it, i) => {
    const {distance: dist, tile } = getTilingDistance(playerPosition, Vector.fromIVector(it.position), it.tiles);
    if (dist<min){
      min = dist;
      minIndex = i;
      minTile = tile;
  ...
  export function findClosestGold(playerPosition:Vector, golds:Array<InteractiveObject>){
  let min = Number.MAX_SAFE_INTEGER;
  let minIndex = -1;
  golds.forEach((it, i) => {
    const dist = Vector.fromIVector(it.position).sub(playerPosition).abs();
